...
ref: https://docs.docker.com/engine/reference/commandline/run/ <br>
...
- docker run -d --> run container in background. Console can be used after the container is started up
  - e.g. docker run -d busybox@latest sleep 60 and do a docker ps (container active for 60secs)
- docker run --rm --> removes the container after it becomes inactive
- docker run --name --> creates a name for the docker. Or else is randomly assigned
  - e.g. docker run --name terence busybox:latest
- docker run -v myvol2:/app --> bind mount a volume. Doing so will have persisting data generated by and used by Docker containers. 
...
- docker run -e --> set environment varibles or overwrite variables that are defined in the Dockerfile of the image you’re running
- docker run [OPTIONS] IMAGE [COMMAND] [ARG...] --> After the image is the command that pass into the docker container
